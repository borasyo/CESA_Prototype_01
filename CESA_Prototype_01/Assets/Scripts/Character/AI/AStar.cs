...
public class AStar : MonoBehaviour {
    ...
    class ANodeMgr
    {
        ...
        public ANodeMgr(Character me, int xgoal, int zgoal)
        {
            _me = me;
        ...
        public ANode OpenNodeStart(int x, int z, int cost, int goal)
        {
            // ノードを取得する.
            var node = GetNode(x, z);
            if (node.IsNone() == false)
	...
	public bool Search (int nTargetNumber)
    {
        ...
        // A-star実行.
        {
            ...
            var mgr = new ANodeMgr(GetComponent<Character>(), pGoal.x, pGoal.z);

			// スタート地点のノード取得
			// スタート地点なのでコストは「0」
			ANode node = mgr.OpenNodeStart(pStart.x, pStart.z, 0, goalIdx);
